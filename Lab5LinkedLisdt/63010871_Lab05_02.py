...
if __name__ == '__main__':
    ...
    for i in inp:
        ...
        if k[0] == 'A':
            lkls.push_back(k[1])
            print("linked list :", lkls)
            print("reverse :", lkls.traverse(rev=True))
        if k[0] == 'Ab':
            lkls.push_front(k[1])
            print("linked list :", lkls)
            print("reverse :", lkls.traverse(rev=True))
        if k[0] == 'I':
            m = k[1].split(':')
            if (int(m[0])<0) or (int(m[0])>int(lkls.size())):
                print("Data cannot be added")
                if not lkls.is_empty():
                    print("linked list :", lkls)
                    print("reverse :", lkls.traverse(rev=True))
                else:
                    print("linked list :")
                ...
                lkls.insert(int(m[0]),m[1])
                print(f'index = {m[0]} and data = {m[1]}')
                print("linked list :", lkls)
                print("reverse :", lkls.traverse(rev=True))
        if k[0] == 'R':
            indexR = lkls.index(k[1])
            if indexR != -1:
                lkls.remove(k[1])
                print(f'removed : {k[1]} from index : {indexR}')
                if not lkls.is_empty():
                    print("linked list :", lkls)
                    print("reverse :", lkls.traverse(rev=True))
                else:

...
class BinarySearchTree:
        ...
        for i in range(nodeAmount):
            ...
            if currentNode.left is None:
                ...
                if count >= len(inputList) - 1:
                    currentNode.left = Node(inputList[index])   # insert number in List to Node
    ...
    def calculateNum(self):       # breadth first search
        queue = Queue()
        queue.enQ(self.root)
        count = 0
...
if len(inp) == k//2+1:
    ...
    tree.calculateNum()              # plus every Value in tree
else:
